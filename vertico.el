    ...
    (let (cand cmd)
      (cond ((and (eolp)
	    ...
	    ((member cmd '(recentf-open recent consult-recent-file))
	     (deletefilehist cand 'recentf-list)
	     (redisp))
	    (t (kill-line arg))))))
...
;;(define-key vertico-map (kbd "S-<iso-lefttab>") nil)
(define-key vertico-map (kbd "<backtab>") 'vertico-insert)
;; xterm + tmux
(define-key vertico-map (kbd "M-[ z")   'vertico-insert)
